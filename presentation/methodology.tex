...
\begin{frame}
    ...
    \begin{figure}[t]
        ...
        \begin{tikzpicture}[node distance=1cm]
            ...
            \node (inst1) [institution, below of=region, xshift=2cm] {Fuel Cycle};
            \node (inst2) [institution, below of=region, xshift=-2cm] {Nation};
            \node (lwr) [facility, below of=inst2] {LWRs};
            \node (mine) [facility, below of=inst1, xshift=-1.5cm] {Mine};
            ...
            \node (ad_rx) [facility, below of=inst1, xshift=1.5cm] {Advanced Reactor};
            \node (wet_stor) [facility, below of=ad_rx] {Wet Storage};
            \node (dry_stor) [facility, below of=wet_stor] {Dry Storage};
            \node (llw_sink) [facility, below of=dry_stor] {LLW Sink};
            \node (hlw_sink) [facility, below of=llw_sink] {HLW Sink};

            \draw [connect] (region) -- (inst1);
            \draw [connect] (region) -- (inst2);
            \draw [connect] (inst2) -- (lwr);
            \draw [connect] (inst1) -- (mine);
            \draw [connect] (inst1) -- (ad_rx);
            \draw [connect] (mine) -- (mill);
            \draw [connect] (mill) -- (conversion);
            \draw [connect] (conversion) -- (enrichment);
            \draw [connect] (enrichment) -- (fuelfab);
            \draw [connect] (ad_rx) -- (wet_stor);
            \draw [connect] (wet_stor) -- (dry_stor);
            \draw [connect] (dry_stor) -- (llw_sink);
            \draw [connect] (llw_sink) -- (hlw_sink);

        \end{tikzpicture}
        \caption{Oranization of agents in the simulations, green shows the region, red shows institutions,
